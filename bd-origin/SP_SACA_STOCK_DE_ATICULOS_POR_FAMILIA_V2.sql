--------------------------------------------------------
--  DDL for Function SP_SACA_STOCK_DE_ATICULOS_POR_FAMILIA_V2
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE FUNCTION "USR_TSI_SUITE"."SP_SACA_STOCK_DE_ATICULOS_POR_FAMILIA_V2" (
 ...
 AS
    ...
    v_stkaduana   VARCHAR2(80) := '';
    v_cod_alm     NUMBER;
BEGIN
/*FORMA DE USO*/
-- SELECT *
-- FROM TABLE(SP_SACA_STOCK_DE_ATICULOS_POR_FAMILIA_V2(54,2022,4,1,-1,0,-1,-1,null,null,null,null,0));
    ...
    v_stkaduana := ', 0 ';
    IF ( pin_indstkaduana <> -1 ) THEN
        v_stkaduana := ', sp000_saca_saldos_aduanas_articulosv2('
                       || pin_id_cia
                       || ',A.TIPINV,A.CODART)';
    END IF;
    ...
    v_sqlselect := 'SELECT rec_stock_por_familia_V2('
                   || v_sqlselect
                   || ')  FROM  ARTICULOS A '
...
END sp_saca_stock_de_aticulos_por_familia_v2;
