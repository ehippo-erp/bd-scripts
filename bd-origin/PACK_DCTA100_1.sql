--------------------------------------------------------
--  DDL for Package Body PACK_DCTA100
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "USR_TSI_SUITE"."PACK_DCTA100" AS
    ...
    BEGIN
        FOR i IN registro LOOP
            BEGIN
                pack_dcta100.sp_update_saldo(pin_id_cia, i.numint, v_mensaje);
    ...
    EXCEPTION
        WHEN pkg_exceptionuser.ex_saldo_en_negativo THEN
            SELECT
                JSON_OBJECT(
                    'status' VALUE 1.1,
                    'message' VALUE 'Se ha calculado un saldo en negativo en la PLANILLA [ '
                                    || pin_numint
                                    || ' ], proceso DETENIDO'
                )
            INTO pin_mensaje
        ...
        WHEN pkg_exceptionuser.ex_saldo_mayor THEN
            SELECT
                JSON_OBJECT(
                    'status' VALUE 1.2,
                    'message' VALUE 'Se ha calculado un saldo mayor al importe del documento en la PLANILLA [ '
                                    || pin_numint
                                    || ' ], proceso DETENIDO'
                )
            INTO pin_mensaje
