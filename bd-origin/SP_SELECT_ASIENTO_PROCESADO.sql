--------------------------------------------------------
--  DDL for Function SP_SELECT_ASIENTO_PROCESADO
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE FUNCTION "USR_TSI_SUITE"."SP_SELECT_ASIENTO_PROCESADO" (
...
BEGIN
    FOR i IN (
        ...
        FROM
            asiendet   d1
            LEFT OUTER JOIN pcuentas   d2 ON d1.id_cia = d2.id_cia
                                           AND ( d1.cuenta = d2.cuenta )
            LEFT OUTER JOIN tlibro     l1 ON l1.id_cia = d1.id_cia
                                         AND ( l1.codlib = d1.libro )
            LEFT OUTER JOIN tdocume    dc ON dc.id_cia = d1.id_cia
                                          AND ( dc.codigo = d1.tdocum )
            LEFT OUTER JOIN usuarios   ur ON ( ur.id_cia = d1.id_cia
                                             AND ur.coduser = d1.usuari )
            LEFT OUTER JOIN asienhea   a ON a.id_cia = d1.id_cia
                                          AND a.periodo = d1.periodo
                                          AND a.mes = d1.mes
                                          AND a.libro = d1.libro
                                          AND a.asiento = d1.asiento
            LEFT OUTER JOIN usuarios   ua ON ( ua.id_cia = a.id_cia
        ...
        PIPE ROW ( rec );
    END LOOP;
END sp_select_asiento_procesado;
