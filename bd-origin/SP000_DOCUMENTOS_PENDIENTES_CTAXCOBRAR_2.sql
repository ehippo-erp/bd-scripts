--------------------------------------------------------
--  DDL for Function SP000_DOCUMENTOS_PENDIENTES_CTAXCOBRAR_2
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE FUNCTION "USR_TSI_SUITE"."SP000_DOCUMENTOS_PENDIENTES_CTAXCOBRAR_2" (
...
--                        OR(' ' || V_WTIPDOCS || ' ' LIKE '% %' || CAST(D.TIPDOC AS VARCHAR(5))||' ' )
--                        OR(','||V_WTIPDOCS||',' LIKE '%,%'||CAST(D.TIPDOC AS VARCHAR(5))||',' )                        
             )
            AND ( ( v_wcodubis = 'XXX' )
                  OR ( d.codubi IN (
                ...
                FROM
                    ...
                    --OR(' '||V_WCODUBIS||' ' LIKE '% %'||CAST(D.CODUBI AS VARCHAR(5))||' ' )
                    --OR(','||V_WCODUBIS||',' LIKE '%,%'||CAST(D.CODUBI AS VARCHAR(5))||',' )
             )
        ...
        IF ( pin_swfactanti = 'S' ) THEN
            ...
            FOR j IN (
                ...
                WHERE
                    ...
                    -- OR(' '||V_WTIPDOCS||' ' IN ' '||CAST(D6.TIPDOC AS VARCHAR(5))||' ' )
                    -- OR(','||V_WTIPDOCS||',' IN ','||CAST(D6.TIPDOC AS VARCHAR(5))||',' )
                     )
                    AND ( ( v_wcodubis = 'XXX' )
                          OR ( d6.codubi IN (
                        ...
                        FROM
                           ...
                           -- OR(' '||V_WCODUBIS||' ' IN ' '||CAST(D6.CODUBI AS VARCHAR(5))||' ' )
                           -- OR(','||V_WCODUBIS||',' IN ','||CAST(D6.CODUBI AS VARCHAR(5))||',' )
                     )
                    AND ( d16.saldo <> 0 )
            ) LOOP
...
END sp000_documentos_pendientes_ctaxcobrar_2;
