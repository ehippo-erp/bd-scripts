--------------------------------------------------------
--  DDL for Procedure SP_COSTO_PROMEDIO
--------------------------------------------------------
set define off;

  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "USR_TSI_SUITE"."SP_COSTO_PROMEDIO" (
...
--    PASO3: OBTENEMOS DATOS DEL KARDEX DEL PERIODO ACTUAL
    FOR kkk IN rec_kardex(pin_id_cia, pin_periodo, pin_tipinv) LOOP
        v_costot01 := 0.0;
        v_costot02 := 0.0;
        v_cantid := 0.0;
        v_codart := kkk.codart;
        ...
        IF -- LOS ARTICULOS DEL TIPO INVENTARIO 99, SE COSTEAN EN 0
            kkk.costea = 'S'
            AND ( kkk.tipinv = 99 OR kkk.tipinv = 7 )
        THEN -- REDUNDANCIA
            v_update := 'S';
            v_costot01 := 0;
            v_costot02 := 0;
        ...
        ELSIF -- SI EN EL MOMENTO DEL CALCULO ( NO CANTIDAD NI COSTO PROMEDIO DE ESE ARTICULO ) Y ES MOTIVO DEVOLUCION
            kkk.costea = 'S'
            AND ac_cantid = 0
            AND kkk.id = 'I'
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        pout_mensaje := 'ERROR, AL PROCESAR EL COSTO PROMEDIO DEL ARTICULO '
                        || v_codart
                        || ' EN EL DOCUMENTO [ '
                        || v_numint
                        || ' - '
