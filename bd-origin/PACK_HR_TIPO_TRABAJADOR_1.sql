--------------------------------------------------------
--  DDL for Package Body PACK_HR_TIPO_TRABAJADOR
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "USR_TSI_SUITE"."PACK_HR_TIPO_TRABAJADOR" AS
    ...
    FUNCTION sp_buscar (
        ...
        PIPELINED
    IS
        v_table datatable_tipo_trabajador;
    BEGIN
        SELECT
            tp.id_cia,
            tp.tiptra,
        ...
        FROM
            tipo_trabajador tp
            LEFT OUTER JOIN pcuentas        p ON p.id_cia = tp.id_cia
                                          AND p.cuenta = tp.cuenta
        WHERE
                tp.id_cia = pin_id_cia
            AND ( pin_tiptra IS NULL
                  OR tp.tiptra = pin_tiptra )
            AND ( pin_nombre IS NULL
                  OR tp.nombre = pin_nombre );
