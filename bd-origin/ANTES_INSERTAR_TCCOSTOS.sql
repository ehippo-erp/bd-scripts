--------------------------------------------------------
--  DDL for Trigger ANTES_INSERTAR_TCCOSTOS
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE TRIGGER "USR_TSI_SUITE"."ANTES_INSERTAR_TCCOSTOS" BEFORE
    INSERT ON "USR_TSI_SUITE".tccostos
    FOR EACH ROW
DECLARE
    v_conteo1  NUMBER := 0;
    v_conteo2  NUMBER := 0;
...
EXCEPTION
    WHEN pkg_exceptionuser.ex_codigo_no_existe_pcuentas THEN
        raise_application_error(pkg_exceptionuser.codigo_no_existe_pcuentas, ' Codigo '||:new.codigo||' no existe en el plan de cuentas.');
    WHEN pkg_exceptionuser.ex_destin_no_existe_pcuentas THEN
        raise_application_error(pkg_exceptionuser.destin_no_existe_pcuentas, ' Destino '||:new.destin||' no existe en el plan de cuentas.');
END;

/
ALTER TRIGGER "USR_TSI_SUITE"."ANTES_INSERTAR_TCCOSTOS" ENABLE;
