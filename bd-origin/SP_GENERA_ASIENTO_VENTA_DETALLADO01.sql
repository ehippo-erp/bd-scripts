--------------------------------------------------------
--  DDL for Procedure SP_GENERA_ASIENTO_VENTA_DETALLADO01
--------------------------------------------------------
set define off;

  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "USR_TSI_SUITE"."SP_GENERA_ASIENTO_VENTA_DETALLADO01" (
...
BEGIN
    ...
    IF pin_tipdoc = 3 THEN
        v_libro := '07';
    END IF;
    IF pin_tipdoc = 7 THEN
        v_libro := '03';
    END IF;
    IF pin_tipdoc = 8 THEN
        v_libro := '02';
    END IF;
    BEGIN
...
/*obtengo ultimo dia del mes */
    ...
    /*Genera asiento por documento*/
        ...
        sp_genera_asiento_venta_detallado_documento01(pin_id_cia, i.numint, v_libro, pin_periodo, pin_mes,
                                                      1,v_item,v_codsunat);
    ...
    sp_contabilizar_asiento(pin_id_cia, v_libro, pin_periodo, pin_mes, 1,
                            pin_coduser, v_msj);
END sp_genera_asiento_venta_detallado01;
